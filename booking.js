...
function startCountdown(exp){const el=$("#quote-countdown");clearInterval(window.__quoteTimer);const tick=()=>{const s=Math.max(0,Math.floor((Date.parse(exp)-Date.now())/1000));el.textContent="Preço garantido por "+Math.floor(s/60)+":"+String(s%60).padStart(2,"0")+" · datas ainda não bloqueadas";if(!s){el.textContent="Cotação expirada · gere uma nova para atualizar o preço";clearInterval(window.__quoteTimer)}};tick();window.__quoteTimer=setInterval(tick,1000)}

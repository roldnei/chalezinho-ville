...
export default async function handler(req,res){
  ...
  const {property_code,check_in,check_out,guests}=req.body||{};
  ...
  if(!url||!key){
    if(process.env.VERCEL_ENV==='preview'){
      ...
      return res.status(201).json({ok:true,hold_id:'preview-'+Date.now().toString(36),expires_at:expires,minutes:HOLD_MINUTES,preview:true});
    }
  ...
  try{
    ...
    const q=new URLSearchParams({
      select:'id,status,hold_expires_at',property_id:'eq.'+property.id,
      check_in:'lt.'+check_out,check_out:'gt.'+check_in,status:'in.(hold,pending_payment,confirmed)'
    });
    ...
    const expires=new Date(Date.now()+HOLD_MINUTES*60000).toISOString();
    const payload={property_id:property.id,check_in,check_out,guests:Math.max(1,Math.min(2,Number(guests)||2)),status:'hold',hold_expires_at:expires};
    const ir=await fetch(base+'/rest/v1/reservations',{method:'POST',headers:{...headers,Prefer:'return=representation'},body:JSON.stringify(payload),signal:AbortSignal.timeout(5000)});
    const inserted=await ir.json().catch(()=>null);
    if(ir.status===409) return res.status(409).json({ok:false,error:'dates_unavailable'});
    if(!ir.ok) return res.status(ir.status).json({ok:false,error:'hold_create_failed'});
